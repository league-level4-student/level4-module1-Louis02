...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	private void setFoodLocation() {
		...
		int r = ran.nextInt(WIDTH);
		Random rran = new Random();
		int rr = rran.nextInt(HEIGHT);
		Location loc = new Location(r, rr);
		...
		while (snake.isLocationOnSnake(foodLocation)) {
			Random raan = new Random();
			int rrr = raan.nextInt(WIDTH);
			Random rann = new Random();
			int rrrr = rann.nextInt(HEIGHT);
			loc = new Location(rrr, rrrr);
			foodLocation = loc;
		}
